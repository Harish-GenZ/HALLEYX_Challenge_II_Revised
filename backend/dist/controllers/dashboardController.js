...
function normalizeWidgetConfigType(type) {
...
function validatePositiveInteger(value, fieldName, minimum) {
...
function validateWidgetConfig(type, rawConfig) {
    ...
    if (type === 'Table') {
        if (rawConfig.columns !== undefined) {
            ...
            sanitizedConfig.columns = Array.from(new Set(rawConfig.columns));
        }
...
function validateWidgetsPayload(rawWidgets) {
    ...
    for (const [index, rawWidget] of rawWidgets.entries()) {
        ...
        if (typeof width === 'string')
            return { error: width };
        if (width > 12)
            return { error: `widgets[${index}].width cannot exceed 12` };
...
const saveDashboardWidgets = async (req, res) => {
    ...
    const validation = validateWidgetsPayload(req.body.widgets);
